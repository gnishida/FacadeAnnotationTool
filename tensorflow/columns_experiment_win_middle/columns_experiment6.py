...
def load_imgs(path_list, column_params, floor_params, use_augmentation = False, augmentation_factor = 1, use_shuffle = False, all_columns = False, horizontal_flip = False, debug = False):
	...
	for file_path in path_list:	
		...
		# horizontal flip
		if horizontal_flip:
			orig_img = numpy.flip(orig_img, 1)
			img = cv2.resize(orig_img, dsize=(WIDTH, HEIGHT), interpolation=cv2.INTER_CUBIC)
...
def train(input_dir, model_dir, num_epochs, learning_late, augmentation_factor, all_columns, output_dir, debug):
	...
	X, Y = load_imgs(path_list, column_params, floor_params, use_augmentation = True, augmentation_factor = augmentation_factor, use_shuffle = True, all_columns = all_columns, horizontal_flip = True, debug = debug)
	print(X.shape)
	if debug: return
