...
def augmentation(x, y_bot, y_top):
    ...
    y_bot = (y_bot * height + shift_v - offset_y) / height
    y_top = (y_top * height + shift_v - offset_y) / height
    if y_bot < 0 or y_bot > 1:
        y = 0

    if y_top < 0 or y_top > 1:
        y = 0
...
def output_img(x, y_bot, y_top, filename):
    print(x.shape)
    img = Image.fromarray(x.astype(numpy.uint8))
    w, h = img.size
    imgdraw = ImageDraw.Draw(img)

    imgdraw.line([(0, h * y_bot), (w, h * y_bot)], fill = "yellow", width = 3)
    imgdraw.line([(0, h * y_top), (w, h * y_top)], fill = "yellow", width = 3)
