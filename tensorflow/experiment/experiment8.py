...
NUM_CHANNELS = 3
NUM_CLASSES = 1
MODEL_FILE_NAME = "experiment8_model.h5"
...
def augmentation(x, y):
	...
	# crop
	shift_h = int(width * 0.025)
	shift_v = int(height * 0.025)
	#shift_h = 4
	#shift_v = 4
	...
	for file_path in path_list:	
		...
		for y in values:
			...
			if use_augmentation:
				...
				img_tmp = cv2.resize(imgx, dsize=(WIDTH, HEIGHT), interpolation=cv2.INTER_CUBIC)
				X[i,:,:,:] = standardize_img(img_tmp)
				Y[i] = actual_y
				i += 1
